                ...
                parameters: {
                    ...
                    properties: {
                        ...
                        excluded_days_for_courses: {
                            type: "string",
                            description:
                                "JSON string mapping course_code to days to exclude for that specific course. " +
...
export async function executeTool(
    ...
    switch (toolName) {
        ...
        case "build_schedule": {
            ...
            let excludedDays: Record<string, string[]> = {};
            if (input.excluded_days_for_courses) {
              try {
                excludedDays = JSON.parse(input.excluded_days_for_courses as string);
              } catch {
                excludedDays = {};
              }
            }
            ...
            const resp = await fetch(`${baseUrl}/api/schedule`, {
                ...
                body: JSON.stringify({
                    ...
                    excludedDaysForCourses: excludedDays,
                    preferences: {
