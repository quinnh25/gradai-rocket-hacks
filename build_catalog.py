...
# --- Configuration ---
# WORKLOAD_CSV_FILE = "test.csv"
WORKLOAD_CSV_FILE = "umich_atlas_workload_checkpoint_p130.csv"
...
def fetch_sections(subject, catalog_nbr, school_code, headers):
    ...
    for sec in sections_raw:
        # The spec uses EnrollmentStatus: "open" / "closed" directly on the section
        enrollment_status = sec.get("EnrollmentStatus", "").lower()
        available = sec.get("AvailableSeats", 0)

        # Meeting is a single object per section in the Sections endpoint (not a list)
        ...
        if meeting_raw:
            ...
            for m in meeting_raw:
                meetings.append({
                    # Spec field is "Times" (combined string e.g. "9:30AM - 10:30AM")
                    "Days": m.get("Days", ""),
                    "Times": m.get("Times", "TBA"),
                    "Location": m.get("Location", "TBA"),
                    # Spec field is "Instructors" (string), not "InstructorName"
                    "Instructor": m.get("Instructors", "Staff"),
                    "StartDate": m.get("StartDate", ""),
                    "EndDate": m.get("EndDate", ""),
                })

        # ClassInstructors is an array of {InstrUniqname, InstrName}
        ...
        sections.append({
            ...
            "SectionType": sec.get("SectionType", ""),       # e.g. "LEC", "LAB", "DIS"
...
def build_unified_catalog():
    ...
    # Build subject→school map once (avoids needing school code per course)
    ...
    for course_code, workload_info in workload_data.items():
        ...
        unified_catalog[course_code] = {
            ...
            "course_title": workload_info.get("course_name", ""),
            "course_description": course_descr,
            "credits": credits,
            "school_code": school_code,
            "term": TARGET_TERM_CODE,
            "metrics": {
                "workload_percent": workload_info["workload_percent"],
            },
            "availability": sections,
        }

        time.sleep(0.3)  # be polite to the gateway
